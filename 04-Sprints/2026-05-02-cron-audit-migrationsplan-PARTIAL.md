...
## Exact Preflight Blocker

- Post-bootstrap latency fix and Phase 1 mutations are blocked by preflight: board open_count=8 and dirty Mission Control worktree.
- Waiting for green preflight or documented incident override before changing crontab/config/routing.

## Phase 2 Read-Only Schema Discovery

- `openclaw.json` Discord guild config currently does not include channel `1491148986109661334`.
- `discord-routing.json` also does not include channel `1491148986109661334`; its fallback routes unknown channels to `main`.
- Runtime channel config schema evidence supports `enabled` and `requireMention` per channel. No clear `ignore`, `noop`, or `notify_only` key was found in the bundled schema search.
- Safest later apply candidate, once preflight is green: add channel `1491148986109661334` to OpenClaw Discord guild config as disabled or require-mention-only, and separately remove/avoid fallback-to-main for notify/system traffic in the routing layer.
- Because schema evidence for `ignore: true` is absent, do not invent that key.

## Phase 3 Read-Only Skill/Tool Finding

- Atlas config lists 28 skills.
- Live `systemPromptReport.skills.entries` for measured Atlas runs only injected 5 skill prompt blocks: `discord`, `model-router`, `session-logs`, `taskflow`, `taskflow-inbox-triage`.
- Therefore the 28 configured skill names are not the primary remaining latency source after the previous slim work.
- Live `systemPromptReport.tools.entries` still exposed 40 tools and about 30290 schema chars.
- Largest avoidable operational bloat: `message`, `video_generate`, `image_generate`, `canvas`, `tts`, `pdf`, media/web fetch tooling. The planned next fix should target `agents.list[id=main].tools.allow`, not another skill-list-only change.

## Phase 4 Read-Only System-Bot Finding

- No `system-bot` agent currently exists in `agents.list`.
- Exact MiniMax model IDs available and authenticated according to `openclaw models list`:
  - `minimax/MiniMax-M2.7`
  - `minimax/MiniMax-M2.7-highspeed`
- Minimal system-bot candidate should copy the lean agent shape from `james`/`efficiency-auditor`, not Atlas.
- Recommended primary for system-bot remains `minimax/MiniMax-M2.7-highspeed` for cron/notification throughput, with fallback `minimax/MiniMax-M2.7` then a single emergency fallback if required.
- Apply is blocked until preflight is green or incident override is documented.

## Phase 5 Read-Only CPU Finding

- Gateway PID `2745535` sampled 10 times over 5 minutes.
- CPU was stable at about `5.8%` with MC health `ok/ok`, `inProgress=0`, `pendingPickup=0`.
- Busy-loop is currently resolved by the earlier task-registry maintenance fix. No further CPU mutation is indicated in this pass.
