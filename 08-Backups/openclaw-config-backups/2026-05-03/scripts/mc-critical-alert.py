...
RESTART_WINDOW = Path(os.environ.get('MC_RESTART_WINDOW', '/tmp/mc-restart-window'))
...
def in_restart_window() -> bool:
    try:
        age = time.time() - RESTART_WINDOW.stat().st_mtime
        return age < 300
...
def recent_mc_down_count(state) -> int:
    now = time.time()
    events = [ts for ts in state.get('mc_down_events', []) if now - float(ts) < 3600]
    state['mc_down_events'] = events
    return len(events)


def remember_mc_down(state) -> int:
    events = state.get('mc_down_events', [])
    events.append(time.time())
    state['mc_down_events'] = [ts for ts in events if time.time() - float(ts) < 3600]
    return len(state['mc_down_events'])


def mc_context_tail() -> str:
    import subprocess
    try:
        proc = subprocess.run(
        ...
        return (proc.stdout or proc.stderr or '').strip()[-1800:]
    except Exception as e:
        return f'journal context unavailable: {e}'
...
def check_mc_down(state):
    ...
    if ok:
        ...
        state.pop('mc_down_events', None)
        return
    if in_restart_window():
        log('INFO', 'MC down check suppressed during controlled restart window')
        return
    ...
    if down_sec >= DOWN_THRESHOLD_SEC and not rate_limited(state, 'mc_down'):
        count = remember_mc_down(state)
        title = 'MC DOWN'
        body = f'Mission Control is unreachable for {int(down_sec/60)}min.\nWatchdog may have failed. Check systemd: `systemctl --user status mission-control`'
        if count > 5:
            title = 'PROLONGED-OUTAGE'
            body = body + '\n\nLast mission-control journal lines:\n```text\n' + mc_context_tail() + '\n```'
        send_discord(
            title,
            body,
        )
