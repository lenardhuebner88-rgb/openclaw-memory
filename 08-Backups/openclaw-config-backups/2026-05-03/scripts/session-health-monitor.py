...
Ignores non-runtime session artifacts:
- .trajectory sidecars
- .checkpoint/.archived/.deleted/.reset snapshots
...
def is_ignored_session_artifact(session_file: Path) -> bool:
    ...
    if ".checkpoint." in name or ".archived." in name or ".deleted." in name or ".reset." in name:
        return True
...
def scan() -> tuple[list[dict], list[str]]:
    ...
    for sessions_dir_str in glob.glob(SESSIONS_GLOB):
        ...
        for session_file in sessions_dir.glob("*.jsonl"):
            if is_ignored_session_artifact(session_file):
                continue
