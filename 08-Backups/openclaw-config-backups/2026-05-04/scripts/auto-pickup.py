...
MISSION_CONTROL_DATA_DIR = Path(
    os.environ.get("MISSION_CONTROL_DATA_DIR", "/home/piet/.openclaw/state/mission-control/data")
)
...
ROTATION_MALFORMED_LOCK_GRACE_SEC = int(os.environ.get("AUTO_PICKUP_ROTATION_MALFORMED_LOCK_GRACE_SEC", "600"))
...
def _find_session_file_by_id(agent: str, short_sid: str):
    """Find the session .jsonl file matching the (truncated) session ID."""
    store = _load_agent_session_store(agent)
    for entry in store.values():
        if not isinstance(entry, dict):
            continue
        sid = str(entry.get("sessionId") or "").strip()
        sf = str(entry.get("sessionFile") or "").strip()
        if sf:
            p = Path(sf)
            stem = p.stem
            if (sid and (sid == short_sid or sid.startswith(short_sid))) or                (stem and (stem == short_sid or stem.startswith(short_sid))):
                if p.exists():
                    return p
    return None


def _rotation_blocked_by_active_lock(session_file: Path) -> tuple[bool, str]:
    """Guard rotation when target session is currently active/in-flight."""
    ...
    try:
        ...
        return False, "lock-disappeared"

    lock_meta = _read_lock_meta(lock_path)
    if _lock_has_spawn_meta(lock_meta):
        if _lock_process_alive(lock_meta):
            return True, f"active-session-lock age={int(lock_age)}s"

        pid_raw = lock_meta.get("pid")
        pid = int(pid_raw) if isinstance(pid_raw, int) or (isinstance(pid_raw, str) and str(pid_raw).isdigit()) else 0
        if pid <= 0 and lock_age < ROTATION_MALFORMED_LOCK_GRACE_SEC:
            return True, f"malformed-session-lock-grace age={int(lock_age)}s"
    elif lock_age < SESSION_LOCK_THRESHOLD_SEC:
        return True, f"fresh-legacy-session-lock age={int(lock_age)}s"

    return False, f"stale-or-dead-lock age={int(lock_age)}s"


def _do_session_rotation(session_file: Path):
    """Rotate one session file: archive it, create fresh bootstrap, update store."""
    ...
    try:
        blocked, blocked_reason = _rotation_blocked_by_active_lock(session_file)
        if blocked:
            log("ROTATION_DEFER", f"file={session_file.name} reason={blocked_reason}")
            return False, f"rotation-deferred:{blocked_reason}"

        agent = session_file.parts[session_file.parts.index("agents") + 1]
        store = _load_agent_session_store(agent)
        session_key = store_entry = None
        for sk, se in store.items():
            if isinstance(se, dict) and str(se.get("sessionFile") or "").strip() == str(session_file).strip():
                session_key, store_entry = sk, se
                break
        if not session_key or store_entry is None:
            return False, f"binding-not-found-in-{agent}-store"
        content_bytes = session_file.read_text(encoding="utf-8", errors="ignore")
        checkpoint.write_text(content_bytes, encoding="utf-8")
        session_file.replace(archived)
        bootstrap = (
            f'{{"type":"message","id":"rotation-boot-{new_session_id[:8]}",'
            f'"createdAt":"{_utc_now_iso()}","message":{{"role":"system",'
            f'"content":{{"type":"text","text":"Session rotation bootstrap '
            f'(auto-pickup live). Previous archived {ts}."}}}}}}\n'
        )
        new_session.write_text(bootstrap, encoding="utf-8")
        ...
        _save_json_file(store_path, store)
        return True, f"rotated-archived-{archived.name}-new-{new_session.name}"
    except Exception as exc:
        return False, f"rotation-error:{exc}"

def _rotate_session(session_id: str, action: str):
    """Find the session file for session_id across all agents and rotate it."""
    agents_to_try = ["main", "sre-expert", "frontend-guru", "efficiency-auditor", "spark", "james"]
    for agent in agents_to_try:
        sf = _find_session_file_by_id(agent, session_id)
        if sf is not None:
            log("ROTATION_EXEC", f"agent={agent} session={session_id} action={action} file={sf.name}")
            return _do_session_rotation(sf)
    return False, "session-file-not-found-in-any-agent-store"
...
def _consume_rotation_signal() -> tuple[str, str]:
    ...
    if existing and existing.get("mode") == ROTATION_CONSUMER_MODE:
        try:
    ...
    if ROTATION_CONSUMER_MODE == "live":
        # ROTATION_LIVE_COMMAND is not set; use built-in rotation logic instead.
        rot_ok, rot_detail = _rotate_session(session_id, action)
        action_event.update({
            "command": f"builtin-rotate",
            "rotation_ok": rot_ok,
            "rotation_detail": rot_detail,
        })
        if not rot_ok:
            is_deferred = isinstance(rot_detail, str) and rot_detail.startswith("rotation-deferred:")
            if is_deferred:
                action_event["status"] = "deferred"
                action_event["rotation_detail"] = rot_detail
                _append_rotation_action_log(action_event)
                log("ROTATION_SIGNAL_DEFER", f"session={session_id} action={action} detail={rot_detail}")
                return "blocked", rot_detail

            action_event["status"] = "failed"
            _append_rotation_action_log(action_event)
            log("ROTATION_SIGNAL_FAIL", f"session={session_id} action={action} detail={rot_detail}")
            return "failed", f"rotation-failed:{rot_detail}"
        action_event["status"] = "consumed"
    else:
        action_event.update({
            "status": "simulated",
