...
Architecture:
  User taps slash-command / button in Discord
   ...
   -> Bot calls Mission Control / OpenClaw APIs
...
MISSION_CONTROL_DATA_DIR = Path(
    os.environ.get('MISSION_CONTROL_DATA_DIR', str(OPENCLAW_ROOT / 'state/mission-control/data'))
)
MISSION_CONTROL_TASKS_FILE = MISSION_CONTROL_DATA_DIR / 'tasks.json'
...
# This legacy Commander shares the same Discord application/token as the
# OpenClaw Gateway/Piet-Bot. Guild-scoped Commander slash commands with names
# like /new, /status, /agents override or race the Gateway's native global
# commands in agent channels. Default to clearing Commander guild commands and
# silently ignoring off-channel interactions so the Gateway can handle them.
COMMANDER_SLASH_SYNC_MODE = os.environ.get('COMMANDER_SLASH_SYNC_MODE', 'clear-guild').strip().lower()
COMMANDER_SILENT_OUTSIDE_CHANNEL = os.environ.get('COMMANDER_SILENT_OUTSIDE_CHANNEL', 'true').strip().lower() not in ('0', 'false', 'no')
...
async def auth_check(interaction: discord.Interaction, require_operator: bool = False) -> bool:
    ...
    if COMMANDER_CHANNEL_ID and interaction.channel_id != COMMANDER_CHANNEL_ID:
        log.warning(
            'ignored command outside commander channel: command=%s channel=%s user=%s',
            getattr(getattr(interaction, 'command', None), 'name', '?'),
            interaction.channel_id,
            uid,
        )
        if not COMMANDER_SILENT_OUTSIDE_CHANNEL:
...
async def on_ready():
    ...
    # IMPORTANT: this legacy Commander shares the Piet-Bot/OpenClaw Gateway
    # application. Do not publish unprefixed guild commands here: guild-scoped
    # /new, /status, /agents, etc. override/race the Gateway's global native
    # commands in every channel. Default mode clears stale Commander guild
    # commands; the Gateway remains responsible for the global slash surface.
    guild = discord.Object(id=GUILD_ID) if GUILD_ID else None
    synced = []
    try:
        if guild and COMMANDER_SLASH_SYNC_MODE in ('clear-guild', 'clear', 'off', 'disabled'):
            bot.tree.clear_commands(guild=guild)
            synced = await bot.tree.sync(guild=guild)
            log.info('cleared commander guild slash commands; gateway global commands remain active')
        elif guild:
            bot.tree.copy_global_to(guild=guild)
            synced = await bot.tree.sync(guild=guild)
            log.warning('synced %d legacy commander guild slash commands (collision risk)', len(synced))
        else:
            log.info('no guild configured; commander slash sync skipped to avoid global command collision')
    except Exception as e:
...
async def cmd_status(interaction: discord.Interaction):
    ...
    try:
        tasks_data = json.loads(MISSION_CONTROL_TASKS_FILE.read_text())
...
class PlanApprovalView(discord.ui.View):
    ...
    async def dispatch_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        # TODO: Atlas dispatch (placeholder: trigger via Mission Control API)
...
async def cmd_help(interaction: discord.Interaction):
    emb = discord.Embed(
        ...
        description=(
            ...
            '`/meeting-debate <topic>` — queue structured agent debate\n'
