...
MEETING_TEMPLATE = Path(os.environ.get('MEETING_TEMPLATE', '/home/piet/vault/99-Templates/template-meeting.md'))
...
def read_meeting_template_body() -> str:
    try:
        text = MEETING_TEMPLATE.read_text(encoding='utf-8')
...
async def send_meeting_ack(interaction: discord.Interaction, mode: str, topic: str) -> None:
    ...
    budget = MEETING_BUDGETS[mode]
    eta = {
        'debate': 'queued; run `/home/piet/.openclaw/scripts/meeting-runner.sh --once` for one manual dispatch cycle',
